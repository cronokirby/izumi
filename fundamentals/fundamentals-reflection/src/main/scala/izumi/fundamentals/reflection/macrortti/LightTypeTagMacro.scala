...
import izumi.fundamentals.reflection.macrortti.LightTypeTag.{ReflectionLock, Shared}
...
class LightTypeTagMacro0[C <: blackbox.Context](override val c: C) extends LTTLiftables[C](c) {
  ...
  @inline def makeWeakTagString[T: c.WeakTypeTag]: c.Tree = {
    ...
    val strRef = serialize(res.ref)(LightTypeTag.binary0Serializer)
    val strShared = serialize(Shared(res.basesdb, res.idb))(LightTypeTag.binary1Serializer)

    q"$lightTypeTag.parse($strRef : _root_.java.lang.String, $strShared : _root_.java.lang.String)"
  }
